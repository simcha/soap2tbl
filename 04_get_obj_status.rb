...
CSV.foreach("obj_list.csv", headers: :first_row) do |obj|
  ...
  response = client.call(:get_obj_status, message: { obj_id: obj_id})

  obj_list = response.body[:get_obj_status_response][:get_obj_status_result][:obj_status]
...
CSV.open("obj_statuses.csv", "wb", headers: all_statuses.first.keys, :write_headers => true) do |csv|
